<?php
    ...
    //SACAMOS LA RESERVACION QUE ESTA SIENDO OCUPADA
        $sqlQueryO = "SELECT nombre,fecha_entrada, fecha_salida FROM reservaciones WHERE id_habitacion  = $habitacion AND estatus = 1  ORDER BY id";

        $result = mysqli_query($conn, $sqlQueryO);
        while ($row = mysqli_fetch_assoc($result)) {
            $row["start"]=$row['fecha_entrada'];
            $row["end"]=$row['fecha_salida'];
            $row["rendering"]='background';
